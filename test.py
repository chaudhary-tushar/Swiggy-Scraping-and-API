...
if __name__=="__main__":
    ...
    city_names=city_name_list[9:15]  #changed from 5 to 9
    # city_names.extend(city_name_list[11:16])
    print("appending total cities = ",len(city_names))

    five=city_url_list[9:15]  ### changed from 5 to 9
    ...
    with open('data.csv','a',encoding='utf-8')as file0:
        ...
        file0.write(f"time taken for parsing and building menu is of {len(city_names)} cities and {len(city_res_links)*len(city_res_links[0])} restaurants is = {runTIME}\n\n")
        file0.close()
